...
The attachment `trade_details.jpg` failed to extract due to OCR failure, resulting in missing critical fields: trade_date, settlement_date, quantity, currency, net_amount, and side.
...
| HOST lookup performed before report generation | No |
| HOST APIs called during report generation | None |
| Other external/HOST APIs called during report generation | None |
| GitHub repository tools used | repository tree lookup; code search; branch list; set active branch; create branch; create file; github |
| Code Executor usage | Used only to determine current UTC date |
| `/answers` called | No |
| Report persisted to GitHub | `reports/2026-05-13/human_review/email_078_human_review_report.md` |

## Warnings

- Missing multiple trade fields
- Attachment extraction failed
- OCR extraction unavailable or failed for trade_details.jpg